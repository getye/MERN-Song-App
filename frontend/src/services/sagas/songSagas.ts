...
function* fetchSongs() {
  try {
    const response: { data: Song[] } = yield call(axios.get, 'https://song-app-cjn9.onrender.com/viewsongs');
...
function* addSong(action: AddSongRequestAction) {
  try {
    const response: { data: Song } = yield call(axios.post, 'https://song-app-cjn9.onrender.com/addsong', action.payload);
...
function* updateSong(action: UpdateSongRequestAction) {
  try {
    ...
    const response: { data: Song } = yield call(axios.put, `https://song-app-cjn9.onrender.com/updatesong/${id}`, song);
...
function* deleteSong(action: DeleteSongRequestAction) {
  try {
    yield call(axios.delete, `https://song-app-cjn9.onrender.com/songs/${action.payload}`);
